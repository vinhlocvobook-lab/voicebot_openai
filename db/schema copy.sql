...
CREATE TABLE IF NOT EXISTS ai_model (
  ...
  model_type              ENUM('text','realtime','transcription') NOT NULL COMMENT 'text = Chat Completions; realtime = voice+text; transcription = phiên âm audio',
...
CREATE TABLE IF NOT EXISTS ai_model_price (
  ...
  -- Giá cho model TRANSCRIPTION tính theo PHÚT (vd whisper-1).
  -- (transcription tính theo token thì dùng lại text_input/text_output/audio_input ở trên)
  per_minute_price         DECIMAL(12,6)     NULL COMMENT 'USD / phút audio (whisper-1...)',
...
CREATE TABLE IF NOT EXISTS call_log (
  ...
  -- Token usage — TRANSCRIPTION (model phiên âm audio khách → text)
  tr_model                 VARCHAR(100)      NULL COMMENT 'Model transcription, vd whisper-1 / gpt-4o-mini-transcribe',
  tr_audio_input_tokens    INT UNSIGNED      NULL COMMENT 'Audio input tokens (model tính theo token)',
  tr_text_input_tokens     INT UNSIGNED      NULL,
  tr_text_output_tokens    INT UNSIGNED      NULL,
  tr_audio_seconds         INT UNSIGNED      NULL COMMENT 'Tổng giây audio đã phiên âm (model tính theo phút, vd whisper-1)',

  -- Chi phí (USD) — chốt lại tại thời điểm cuộc gọi
  cost_total_usd           DECIMAL(12,6)     NULL COMMENT 'Tổng = realtime + summary + transcription',
  cost_realtime_usd        DECIMAL(12,6)     NULL,
  cost_summary_usd         DECIMAL(12,6)     NULL,
  cost_transcription_usd   DECIMAL(12,6)     NULL,

  -- Bản giá ĐÃ ÁP DỤNG cho cuộc gọi (truy ngược giá lịch sử dù sau này đổi giá).
  -- Mỗi cuộc gọi dùng tới 3 model: realtime (cuộc gọi) + summary (tóm tắt) + transcription (phiên âm).
  rt_price_id              INT UNSIGNED      NULL COMMENT 'FK ai_model_price — bản giá realtime đã dùng',
  sum_price_id             INT UNSIGNED      NULL COMMENT 'FK ai_model_price — bản giá summary đã dùng',
  tr_price_id              INT UNSIGNED      NULL COMMENT 'FK ai_model_price — bản giá transcription đã dùng',
  ...
  KEY idx_call_rt_price (rt_price_id),
  KEY idx_call_sum_price (sum_price_id),
  KEY idx_call_tr_price (tr_price_id),
  CONSTRAINT fk_call_rt_price FOREIGN KEY (rt_price_id)
    REFERENCES ai_model_price (id) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT fk_call_sum_price FOREIGN KEY (sum_price_id)
    REFERENCES ai_model_price (id) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT fk_call_tr_price FOREIGN KEY (tr_price_id)
    REFERENCES ai_model_price (id) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
...
INSERT INTO ai_model (model_key, description, model_type, approx_cost_per_min_usd, is_active) VALUES
  ...
  ('whisper-1',                    'Whisper - transcription (tính theo phút)',          'transcription', NULL, 1),
  ('gpt-4o-transcribe',            'GPT-4o transcribe (tính theo token)',               'transcription', NULL, 1),
  ('gpt-4o-mini-transcribe',       'GPT-4o mini transcribe (tính theo token)',          'transcription', NULL, 1)
...
-- Giá cho model TRANSCRIPTION
-- ⚠️ Số dưới đây là mức tham khảo phổ biến — HÃY ĐỐI CHIẾU lại với trang giá
--    OpenAI bạn đang dùng trước khi áp dụng (giá transcription hay thay đổi).
--
-- whisper-1: tính theo PHÚT (~0.006 USD/phút)
INSERT INTO ai_model_price
  (model_id, per_minute_price, effective_from, is_current, source, note)
SELECT id, 0.006000, '2026-06-01 00:00:00', 1,
       'https://developers.openai.com/api/docs/pricing', 'updated 2026-06 — CẦN ĐỐI CHIẾU'
  FROM ai_model WHERE model_key = 'whisper-1';

-- gpt-4o-transcribe: tính theo TOKEN (audio_input + text_input + text_output)
INSERT INTO ai_model_price
  (model_id, audio_input_price, text_input_price, text_output_price,
   effective_from, is_current, source, note)
SELECT id, 6.000000, 2.500000, 10.000000, '2026-06-01 00:00:00', 1,
       'https://developers.openai.com/api/docs/pricing', 'updated 2026-06 — CẦN ĐỐI CHIẾU'
  FROM ai_model WHERE model_key = 'gpt-4o-transcribe';

-- gpt-4o-mini-transcribe: tính theo TOKEN
INSERT INTO ai_model_price
  (model_id, audio_input_price, text_input_price, text_output_price,
   effective_from, is_current, source, note)
SELECT id, 3.000000, 1.250000, 5.000000, '2026-06-01 00:00:00', 1,
       'https://developers.openai.com/api/docs/pricing', 'updated 2026-06 — CẦN ĐỐI CHIẾU'
  FROM ai_model WHERE model_key = 'gpt-4o-mini-transcribe';
